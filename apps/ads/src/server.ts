...
/** The full request handler, exported so the Smart 1 Suite can mount this
 *  app inside one unified server. Standalone mode is unchanged. */
...
// Inside the suite the host server owns the port; everything else above
// (worker loop, watchdog, retention, self-health) still runs at import.
if (!process.env.SUITE_MODE) server.listen(PORT, HOST, () => {
